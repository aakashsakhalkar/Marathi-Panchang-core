import { getJulianDay } from './ayanamsha';
...
function formatTimeString(date: Date, offsetHours: number): string {
...
export function calculateSunTimes(
...
): { sunriseStr: string; sunsetStr: string; solarNoonStr: string; dayLengthStr: string; sunriseDate: Date; sunsetDate: Date; solarNoonDate: Date } {
  ...
  if (UT_sunrise < 0) UT_sunrise += 24;

  let UT_sunset = (T_sunset - lngHour) % 24;
  if (UT_sunset < 0) UT_sunset += 24;
