import { getJulianCenturies, getLahiriAyanamsha } from './ayanamsha';
 ...
 */
export function getSunSayanaLongitude(julianDay: number): number {
 ...
 */
export function getMoonSayanaLongitude(julianDay: number): number {
  const T = getJulianCenturies(julianDay);
 ...
 */
export function getPlanetaryPositions(julianDay: number): PlanetaryPositions {
  const ayanamshaDeg = getLahiriAyanamsha(julianDay);
